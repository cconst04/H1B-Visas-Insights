...
```{r}
head(df$WAGE_RATE_OF_PAY_FROM_1)
df[df$WAGE_RATE_OF_PAY_FROM_1<=500000,]%>% ggplot(aes(x = WAGE_RATE_OF_PAY_FROM_1)) + geom_histogram()
```
...
df$income_group <-discretize(df$WAGE_RATE_OF_PAY_FROM_1,method = "fixed",breaks=breaks,infinity = TRUE)
